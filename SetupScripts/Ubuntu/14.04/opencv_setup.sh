...
cmake -D WITH_OPENGL=ON -D WITH_OPENNI=OFF -D WITH_OPENNI2=ON -D WITH_QT=OFF -DOPENCV_EXTRA_MODULES_PATH=/frameworks/opencv/modules/contrib/modules -DBUILD_opencv_legacy=OFF ..	#OpenNI2 clashes with OpenNI in OpenCV - only one per time could be enabled
